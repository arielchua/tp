...
public class FilterCommandParser implements Parser<FilterCommand> {
    ...
    public FilterCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_COURSEID);
        ...
        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_COURSEID);

        CourseId courseId = ParserUtil.parseCourseId(argMultimap.getValue(PREFIX_COURSEID).get());

        return new FilterCommand(new CourseIdMatchesPredicate(courseId));
    }
